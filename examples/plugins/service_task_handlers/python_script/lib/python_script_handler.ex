defmodule Examples.ServiceTaskHandlers.PythonScript.PythonScriptHandler do
  @moduledoc """
  Async Service Task handler that runs an allowlisted Python script with JSON
  ...
  @default_script "echo.py"

  @doc "Parks the FNI and runs the allowlisted Python script asynchronously."
  @impl true
  def handle_enter(_flow_node, token, handler_context) do
    flow_node_instance_id = handler_context.flow_node_instance_id
    facade = PythonScriptFacadeStore.get()
    ...
    case ScriptSandbox.execute(script_path, payload, execution_options()) do
      ...
      {:ok, _non_map} ->
        facade.service_tasks.fail_async.(
          ...
          "Python script stdout must decode to a JSON object."
        )
      ...
      {:error, :execution_timeout} ->
        facade.service_tasks.fail_async.(
          flow_node_instance_id,
          "execution_timeout",
          "Python script exceeded the configured timeout."
        )
      ...
      {:error, {:script_failed, exit_status, output_binary}} ->
        facade.service_tasks.fail_async.(
          flow_node_instance_id,
          "script_failed",
          "Python script exited with status #{exit_status}: #{truncate_output(output_binary)}"
        )
      ...
      {:error, :python3_not_found} ->
        facade.service_tasks.fail_async.(
          ...
          "The python3 interpreter was not found on PATH."
        )
      ...
      {:error, reason} ->
        facade.service_tasks.fail_async.(
          ...
          "Python script could not be executed: #{inspect(reason)}"
        )
    end
  end
  ...
  defp execution_options do
    ...
    timeout_milliseconds =
      Application.get_env(:python_script_example, :timeout_milliseconds, 30_000)
