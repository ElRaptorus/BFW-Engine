defmodule Examples.BusinessRules.DecisionServiceSmokeTester.FacadeStore do
  @moduledoc """
  Agent-backed stash for the `BfwEngine.EngineFacade` received during `on_load/1`
  so the smoke tester worker can reach wired `facade.decisions.*` closures from `on_ready/1`.
  """
  ...
  @doc "Stores the engine facade from plugin load so on_ready can start the worker."
