...
This starter wires [`EvilEngine.Plugin.NamedScript`](https://github.com/ElRaptorus/ThomasTheDaemonEngine/blob/main/apps/engine_sdk/lib/evil_engine/plugin/named_script.ex) to files on the operator’s machine. The BPMN carries the relative path in the standard `<bpmn:script>` body while `<evil:scriptRef>local_script</evil:scriptRef>` selects this handler.
...
2. **The engine survives bad scripts** — malformed output, timeouts, and non-zero exit codes surface as `{:error, _}` to the Script Task handler; the FNI transitions `:fatal` while the engine keeps running (see [script-tasks.md](https://github.com/ElRaptorus/ThomasTheDaemonEngine/blob/main/docs/guides/handbook/script-tasks.md)).
...
- [`EvilEngine.EngineFacade`](https://github.com/ElRaptorus/ThomasTheDaemonEngine/blob/main/apps/engine_sdk/lib/evil_engine/engine_facade.ex)
- [plugins architecture](https://github.com/ElRaptorus/ThomasTheDaemonEngine/blob/main/docs/architecture/plugins.md)
