defmodule Examples.Plugins.Shared.ScriptSandbox do
  @moduledoc """
  ...
  Supported extensions: `.py` (`python3`), `.js` (`node`), `.sh` (`bash`).
  """
  ...
  @spec execute(String.t(), map(), execution_options()) :: {:ok, term()} | {:error, term()}
    ...
    case System.find_executable("sh") do
      ...
      posix_shell_path ->
        temporary_stdin_file_path =
          Path.join(
            System.tmp_dir!(),
            "script-sandbox-stdin-#{:erlang.unique_integer([:positive])}.json"
          )
