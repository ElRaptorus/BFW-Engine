...
| `interrupted` | Interrupted by another BPMN element (Boundary Event, Terminate End Event). Error End Event siblings use `:error`, not `:interrupted`. |
...
| `escalated` | Uncaught escalation reached the root — business outcome, not retryable |
| `compensated` | Finished after a Compensation End Event — business outcome, not retryable |
| `cancelled` | Transaction subprocess cancelled via Cancel End Event — business outcome, not retryable |
...
PIs in `fatal`, `aborted`, or `error` state can be retried via the [Retry](retry.md) mechanism.
...
2. All remaining active/waiting sibling FNIs are set to `:error`
...
PIs that reach `fatal`, `aborted`, or `error` state are not permanently stuck. The engine provides a [Retry](retry.md) mechanism that allows operators to:

- Retry a PI from the Start Event or from a specific checkpoint
...
See the [Retry](retry.md) guide for full details.

## Related

- [Error End Events](error-end-events.md) -- throwing modeled BPMN errors
- [Retry](retry.md) -- recovering from fatal, aborted, and error states
