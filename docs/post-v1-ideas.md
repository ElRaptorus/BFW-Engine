...
Allow multiple engine nodes to run as one system. v1 is single-node: timers, PI supervision, and message/signal subscriptions live in that node’s memory. Planning starts after a successful v1 release.
...
5. Multi-node message and signal routing (today a publish reaches only same-node subscriptions).
6. `docker-compose.cluster.yml` for local N-node development.

**Done when** an N-node engine runs the full conformance corpus, with nodes entering and leaving mid-run without data loss.
...
`<bfw:tokenMaxBytes>` or per-route caps. v1 `BFE_TOKEN_MAX_BYTES` is engine-global.
