...
rustler::init!("Elixir.FeelNif");

/// Build a FeelScope from an Elixir context map, suitable for both
/// parsing (name resolution) and evaluation (value binding).
fn build_scope_from_context(env: Env, context: Term) -> Result<FeelScope, String> {
    let feel_ctx = term_to_feel_context(env, context)?;
    Ok(FeelScope::from(feel_ctx))
}

/// Parse a FEEL expression string with a context that provides variable names.
/// Returns {:ok, resource} or {:error, reason}.
#[rustler::nif(schedule = "DirtyCpu")]
fn compile<'a>(env: Env<'a>, expression: String, context: Term<'a>) -> NifResult<Term<'a>> {
    let scope = match build_scope_from_context(env, context) {
        Ok(s) => s,
...
/// Evaluate a previously compiled expression against a context map.
...
/// Parse and evaluate a FEEL expression in one shot.
/// Returns {:ok, value} or {:error, reason}.
#[rustler::nif(schedule = "DirtyCpu")]
...
/// Evaluate a FEEL unary test against an input value.
/// Wraps the test in a FEEL `in` expression for proper evaluation.
/// Returns {:ok, value} or {:error, reason}.
#[rustler::nif(schedule = "DirtyCpu")]
