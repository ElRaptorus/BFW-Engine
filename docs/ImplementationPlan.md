...
| TIM-D1 | Follow-on after Phase 3/4 | **Operational table `timer_start_schedules` (Ash + `EvilEngine.Persistence.TimerStartScheduleAdapter`).** Production `config/config.exs` sets `:core_timers, :persistence_module` to that adapter. Test env keeps `EvilEngine.Timers.Persistence.NoOp`. `ExecutionCase` switches to the adapter. PI-scoped catch/boundary timers stay in FNI `type_properties` plus Scheduler ETS. There is no `engine_timers` table. | Cycle Timer Starts must survive restart; PI-scoped timers already persist on the FNI. A hybrid `engine_timers` table would duplicate FNI state. |
...
├── peripheral_persistence/  # Peripheral — Ash resources + AshPostgres. `RetentionRunner` is Phase 7 (not shipped). Manual purge is planned REST under process-instances, not a GraphQL mutation.
...
| `peripheral_persistence` | Ash resources (`Processes`, `ProcessInstances`, `FlowNodeInstances`, `ProcessInstanceEvents`, …). The built-in `DatabaseSink` was removed. `RetentionRunner` is Phase 7 (not shipped). | The stored shapes; consumed by `api_web` (GraphQL reads) via Ash. Core never imports these |
...
- **Per-table retention granularity for engine-audit tables.** One knob — `EVIL_RETENTION_ENGINE_AUDIT_DAYS` — covers the four engine-audit tables (`messages`, `pending_messages`, `signals`, `pending_signals`). There is no `EVIL_RETENTION_MESSAGES_DAYS` / `EVIL_RETENTION_SIGNALS_DAYS` / etc. Operators who genuinely need asymmetric per-table retention (e.g. "keep messages for 1 year, signals for 30 days") register a plugin sink subscribing to `Event.EngineAuditPurged` to ship deleted rows to external storage with custom per-table retention rules there.
- **Manual purge mutation for engine-audit tables.** The planned PI-scoped REST purge has no engine-audit equivalent in v1. `purgeEngineAudit(olderThan, tables, dryRun, batchSize)` was considered and deferred on surface-area grounds — operators wanting ad-hoc engine-audit cleanup set `EVIL_RETENTION_ENGINE_AUDIT_DAYS` temporarily low or run direct SQL under the admin DB role. Revisit post-v1 if operational demand for one-off selective engine-audit purges materializes. There is no `purgeProcessInstances` GraphQL field.
- **PI-cascade deletion of engine-audit rows.** When a PI is purged (by retention or manual), the corresponding `messages` / `signals` rows that *happened to reference* that PI via their `correlations[]` / `origin` JSONB are **not** deleted with it. The relation is by JSONB content, not by foreign key, and broadcast / cross-PI semantics make "which PI owns this row?" ambiguous anyway. Engine-audit rows age out on their own clock retention or persist until manually cleaned up. This is intentional — a published message is an engine-wide event, and deleting its audit record because one of its 5 recipients was purged would misrepresent history.
