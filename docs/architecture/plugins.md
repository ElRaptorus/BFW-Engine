...
| Engine API consumers (Studio, dashboards, CLIs) | `@evil/engine-client` (npm) — hand-curated TS wrapper on top of OpenAPI Generator + graphql-codegen output + similar auto-generated packages for other major languages | Typed client for REST triggers + GraphQL queries/subscriptions. Additionally ships `moddle/evil.json`, a `bpmn-moddle` descriptor for the `evil:*` XML namespace, so JS/TS consumers using `bpmn-js` / `bpmn-moddle` (Studio debugger, modeler, live overlays) parse `evil:*` extensions in raw `bpmnXml` identically to the engine. The descriptor is **auto-generated at CI time** from the `EvilEngine.BPMN.Model.*` Elixir struct definitions — it cannot drift from the parser |

**Studio's engine extensions** will depend **only** on `@evil/engine-client`. No direct SQL/PubSub/gRPC coupling.

**Moddle-descriptor build pipeline.** A Mix task `mix evil.gen.moddle_descriptor` introspects the `%EvilEngine.BPMN.Model.*{}` struct field definitions and emits `moddle/evil.json` in the `@evil/engine-client` source tree. CI runs the task on every merge to `main`; a diff-guard step fails the build if the generated file is out of sync with the current struct definitions. This keeps the two sides of the Elixir ↔ JS boundary bit-for-bit synchronized without any runtime coupling.
