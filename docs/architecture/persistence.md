...
│  → BfwEngine.Persistence.ReadRepo (pool_size: N₁)     │
├────────────────────────────────────────────────────────┤
│  PI/FNI writes + deploy + retry + resume writes         │
│  → BfwEngine.Persistence.Repo (pool_size: N₂)         │
├────────────────────────────────────────────────────────┤
...
|----------------|-------------|
| `:read` | `BfwEngine.Persistence.ReadRepo` |
| `:mutate` | `BfwEngine.Persistence.Repo` |
...
|------|---------|---------|-----------|
| Write | `BFE_DB_POOL_SIZE` | 100 | Execution writes are individually fast but massively concurrent |
| Read | `BFE_DB_READ_POOL_SIZE` | 50 | GraphQL queries are heavier but far less frequent |
| Total | — | 150 | 2:1 write-to-read ratio reflects workload asymmetry |

Size Postgres with `max_connections >= (write + read) * engine_nodes + 20`. Production defaults (100 + 50) already exceed Postgres's default `max_connections` of 100; a single-node install needs at least 170 (recommend 200). `config/dev.exs` uses a small local pool. `config/test.exs` defaults to the Ecto sandbox (`pool_size: schedulers * 2`); load tests set `BFE_LOAD_TEST_POOL=1` for a real `ConnectionPool` of 50 write / 25 read (`BFE_LOAD_TEST_POOL_SIZE`). See [testing.md](./testing.md) (ExUnit and CI constraints). That total (75) stays under GitHub Actions' stock Postgres `max_connections` of 100. Production 100+50 would exceed that service-container limit. `scripts/create-test-db.sh` starts Postgres with `max_connections=200` so a local Docker database can host the load-test pools with headroom.
...
|-----------|---------|---------|-------------|
| Queue target | `BFE_DB_QUEUE_TARGET` | 100ms | CoDel target latency |
| Queue interval | `BFE_DB_QUEUE_INTERVAL` | 2000ms | CoDel measurement interval |
| Checkout timeout | `BFE_DB_CHECKOUT_TIMEOUT` | 15,000ms | Max wait for a connection |
| Checkout retries | `BFE_DB_CHECKOUT_RETRIES` | 3 | DBConnection Layer 1 retries |
...
`BfwEngine.Telemetry.DbQueryHandler` attaches to Ecto query telemetry events from both repos and re-emits standardized metrics:

| Metric | Type | Tags |
|--------|------|------|
| `bfw_engine.db.query.queue_time_ms` | distribution | `repo` |
| `bfw_engine.db.query.total_time_ms` | distribution | `repo` |
| `bfw_engine.db.query.count` | counter | `repo` |

A warning log is emitted when `queue_time` exceeds the configurable threshold (`BFE_DB_QUEUE_TIME_WARNING_MS`, default 500ms).

### Pool metrics

Sampled every 10s by the telemetry poller via `BfwEngine.Telemetry.Measurements.db_pool_stats/0`:

| Metric | Type | Tags |
|--------|------|------|
| `bfw_engine.db.pool.size` | last_value | `repo` |
| `bfw_engine.db.pool.checked_out` | last_value | `repo` |
| `bfw_engine.db.pool.idle` | last_value | `repo` |
...
| Module | Path |
|--------|------|
| `BfwEngine.Persistence.Repo` | `apps/peripheral_persistence/lib/bfw_engine/persistence/repo.ex` |
| `BfwEngine.Persistence.ReadRepo` | `apps/peripheral_persistence/lib/bfw_engine/persistence/read_repo.ex` |
...
| `BfwEngine.Persistence.RepoRouter` | `apps/peripheral_persistence/lib/bfw_engine/persistence/repo_router.ex` |
| `BfwEngine.Persistence.ExecutionAdapter` | `apps/peripheral_persistence/lib/bfw_engine/persistence/execution_adapter.ex` |
| `BfwEngine.Persistence.ProcessInstancePurge` | `apps/peripheral_persistence/lib/bfw_engine/persistence/process_instance_purge.ex` — Mix/eval PI-tree hard-delete; retry cascade |
| `BfwEngine.Persistence.Release` | `apps/peripheral_persistence/lib/bfw_engine/persistence/release.ex` — migrate / ensure_partitions / purge_retention |
| `mix bfw.retention.purge` | `apps/peripheral_persistence/lib/mix/tasks/bfw.retention.purge.ex` |
| `BfwEngine.Persistence.MessagePersistenceAdapter` | `apps/peripheral_persistence/lib/bfw_engine/persistence/message_persistence_adapter.ex` |
...
1. Point `ReadRepo` at the replica's connection string via a separate `BFE_DB_READ_URL` env var
