...
- When testing cancellation in the shared-connection Ecto sandbox, drive the join with branches that are **idle-waiting** (e.g. user tasks completed explicitly) before the fire. Killing an FNI that is mid-DB-write on the single shared test connection can tear the connection down and surface as a spurious `DBConnection.OwnershipError` or `DBConnection.ConnectionError` — a test artifact, not an engine bug (in production each process has its own pooled connection). Tests that interrupt in-flight work should `poll_pi_state/3` after `wait_for_process_instance/2` rather than a single `assert_pi_state!/2`. `poll_pi_state/3` must restore the shared sandbox on retries that did not observe a PI row (P82). Do **not** check out the sandbox with `sandbox: false` to paper over this — concurrent handler Tasks then share one auto-commit connection and FNIs go `fatal`.
...
**Correct approach:** Classify connection-closed / not-the-owner / `in_failed_sql_transaction` errors as sandbox retries in `DbAssertions`. Restore `{:shared, test_pid}` on every `poll_pi_state` retry that did not observe a PI row. When the fixture itself cancels in-flight script (or other writing) FNIs, **do not poll Postgres for terminal state** — the wrapping sandbox transaction may already be aborted, so the row is gone for the rest of the test. Wait for `ProcessInstanceStateChanged` / `AdHocSubProcessCompleted` on the in-memory `EventCollector` instead. Do not switch `ExecutionCase` to `sandbox: false` (P79).
