...
2. **`EngineEventBus`** — the single public fan-out surface for every typed `EvilEngine.Types.Event.*` payload emitted anywhere in the engine. Wraps the PubSub layer with a sink-routing stage that dispatches each event to every registered `@behaviour EvilEngine.Plugin.EventSink` ([plugins.md](./plugins.md)). This is the only channel through which observability, audit logging, and external integrations ever see engine events.
...
Built-in sinks are automatically registered at application boot by `EvilEngine.Events.SinkRegistrar`, a one-shot `Task` child of the `core_events` Application supervisor. It runs after `EngineEventBus` starts.
...
| Auth provider | `@behaviour EvilEngine.Plugin.AuthProvider` | Unique (singleton, first-writer wins) |
...
…and are dispatched identically to the built-in sinks. Plugins MUST NOT call `EvilEngine.Plugin.Registry.register/2` directly — the registry is private to `peripheral_plugins`; only the engine-injected facade may write to it.
...
- `[:evil_engine, :fni, :state_change]` — FNI transitions (`running → waiting`, etc.). Metadata includes `process_instance_id`.
...
- `Event.EngineOverloaded` — Emitted when the engine's load level crosses *upward* into `:elevated` or `:critical`. Published via `EngineEventBus` only (no `:telemetry.execute/3` pairing). The poller in `EvilEngine.Telemetry.Measurements` detects threshold crossings and publishes only on transitions, not every tick. Fields: `level` (`:elevated` | `:critical`), `active_process_instances`, `limit`, `occurred_at`.
...
- `Event.ProcessInstanceRetried` — Emitted by `Execution.retry_process_instance/1` after successfully starting the retry gen_statem. `process_instance_id` is the root PI, `target_process_instance_id` is the PI the user targeted. Paired with `[:evil_engine, :process_instance, :retried]` telemetry event. `version`, `previous_version`, and `new_version` are **process version UUIDs** (not `evil:version` deployment strings); `previous_version`/`new_version` are `nil` when no version migration. `process_model_id` is the BPMN process ID string (resolved from ModelCache). Fields: `process_instance_id`, `target_process_instance_id`, `process_model_id`, `version`, `previous_state`, `previous_version`, `new_version`, `reset_to_flow_node_instance_id`, `retried_by`, `occurred_at`.
...
Selected `EvilEngine.Types.Event.*` structs published via `EngineEventBus`. WebSocket envelopes use camelCase keys. Opaque payload subtrees pass through unchanged.
