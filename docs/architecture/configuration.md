...
| `EVIL_RETENTION_ENGINE_AUDIT_DAYS` | **Phase 7 (planned).** Max age, in days, for engine-level audit table rows before they become eligible for automated purge by Pass B. Applies, with one cutoff, to tables that exist today: `messages`, `pending_messages` (terminal states only — `delivered`/`expired`/`cancelled`; `pending` rows are live state and never swept), `signals`, `pending_signals` (same terminal-state rule). There is no `pending_escalations` table. Dedicated `escalations`, `compensations`, and `engine_timers` tables are specified but **not migrated** — Pass B must not DELETE them until they exist. Unset = never auto-purge engine-level audit rows | *(unset)* |
...
| `EVIL_PARTITION_AHEAD_MONTHS` | Number of future monthly partitions the `mix evil.partitions.ensure` boot hook creates ahead of time for the tables in `EvilEngine.Persistence.Partitions`: `process_instance_events`, `data_object_writes`, `messages`, `pending_messages`, `signals`, `pending_signals`. There is no `pending_escalations` table. `escalations` / `compensations` are specified but not partitioned today. At least 1 is enforced regardless of configured value | `3` |
...
`engine_timers` is **specified, not migrated**. PI-scoped timer state lives in FNI `type_properties` and Scheduler ETS. When (if) an `engine_timers` table lands, it should remain unpartitioned: `fire_at` can be arbitrarily far-future for scheduled cycle timers, and an immutable `created_at` column would add schema churn without meaningful benefit at realistic volumes.
