...
| `EVIL_DB_POOL_SIZE` | Write connection pool size | `20` |
| `EVIL_DB_READ_POOL_SIZE` | Read connection pool size | `10` |
...
| `EVIL_ESCALATION_PENDING_TTL` | How long a `pending_escalations` row created by the scope-chain walker's "root uncaught" outcome survives in `state='pending'` ([routing.md](./routing.md) §3.5.7.1). **Observability-only hold** — does NOT defer the throw-element-aware terminal state of the originating or any ancestor PI (those are decided immediately by the walker). Drains when a late-registering Escalation Boundary / Event-Subprocess-Start for the same `escalation_code` registers within TTL; transitions to `expired` otherwise. Accepts ISO 8601 duration. Set to `PT0S` to disable the hold entirely (no `pending_escalations` rows written, `escalations.outcome` stays `uncaught_root`/`uncaught_intermediate_throw_noop` — equivalent to pre-pending-escalation-hold behavior) | `PT60S` |
...
| `EVIL_RETENTION_RUN_INTERVAL` | How often the `RetentionRunner` GenServer wakes up to apply retention policies. ISO 8601 duration. Only meaningful if at least one retention-days var below is set | `PT1H` |
| `EVIL_RETENTION_BATCH_SIZE` | Max number of PIs purged per transaction by the `RetentionRunner`. The runner loops until either no more eligible rows remain or the per-tick batch budget is exhausted | `500` |
...
| `EVIL_RETENTION_ENGINE_AUDIT_DAYS` | Max age, in days, for engine-level audit table rows before they become eligible for automated purge by the `RetentionRunner`'s second per-tick pass. Applies, with one cutoff, to `messages`, `pending_messages` (terminal states only — `delivered`/`expired`/`cancelled`; `pending` rows are live state and never swept), `signals`, `pending_signals` (same terminal-state rule), `escalations`, `pending_escalations` (same terminal-state rule), `compensations`, and `engine_timers` (terminal states only — `fired`/`cancelled`). Unset = never auto-purge engine-level audit rows. The runner boots if at least one `EVIL_RETENTION_*_DAYS` var is set (PI-scoped or engine-audit), so enabling only this one is a valid configuration for operators who only care about engine-wide audit cleanup | *(unset)* |
...
| `EVIL_PENDING_ESCALATIONS_KEEP_AFTER_TRANSITION` | Same semantics as the two above, applied to `pending_escalations`. `true` = keep late-catch audit for retention; `false` = physical delete on transition. In practice most operators leave this `true` — uncaught escalations are rare enough that an audit trail of late-catch attempts is valuable even if storage-tight deployments flip the message/signal knobs to `false` | `true` |
| `EVIL_PARTITION_AHEAD_MONTHS` | Number of future monthly partitions the `mix evil.partitions.ensure` boot hook creates ahead of time for `process_instance_events` and `data_object_writes`, plus `messages` / `pending_messages` / `signals` / `escalations` / `compensations`, plus `pending_signals` / `pending_escalations`. At least 1 is enforced regardless of configured value | `3` |
| `EVIL_TOKEN_MAX_BYTES` | **Hard payload cap** applied to the canonicalized JSON byte size of every user-supplied payload across: FNI output tokens via `write_result/2`, Data Object values at DOA-commit time (DOA-only: check runs when the engine materializes each `dataOutputAssociation` post-`onFinished`), published messages/signals/escalations via the PI facade + API trigger surfaces, PI `started_with_context` at start, User Task completion results, async Service Task completion/fail payloads via `engine_facade.finish_async_service_task/2` and `fail_async_service_task/3` (../ImplementationPlan.md §3.6 / [plugins.md](./plugins.md) §9.2.5) — **no dedicated public REST path** for async plugin callbacks; cap is enforced on the facade / GraphQL mutations when those exist. Overflow → structured `{:error, :payload_too_large, size, limit}` from the facade; causing FNI transitions to `fatal`; HTTP/GraphQL endpoints return HTTP 413 / `extensions.code = "PAYLOAD_TOO_LARGE"` before any engine-side work runs. Minimum enforced `1024` (1 KiB); no max — operators running legitimately large-payload workloads can raise this arbitrarily. Configurable for the **entire engine**; no per-process/per-endpoint override in v1 | `65536` (64 KiB) |
...
EVIL_DB_POOL_SIZE=20
EVIL_DB_READ_POOL_SIZE=10
...
|---|---|---|---|
| `process_instance_events` | `occurred_at` | 1 | DB event sink is on |
| `data_object_writes` | `created_at` | 1 | Always |

**Deferred to later phases:**

| Table | Phase | Partition column |
|---|---|---|
| `messages` | 2 | `published_at` |
| `pending_messages` | 2 | `published_at` |
| `signals` | 2 | `published_at` |
| `pending_signals` | 2 | `published_at` |
| `escalations` | 3 | `published_at` |
| `pending_escalations` | 3 | `published_at` |
| `compensations` | 4 | `triggered_at` |
...
`engine_timers` is deliberately **not** partitioned: `fire_at` can be arbitrarily far-future for scheduled cycle timers, and an immutable `created_at` column would add schema churn without meaningful benefit at realistic volumes (~2 timers per PI, `armed`-state working set cascade-deleted with its PI). Retention for `engine_timers` runs as row-by-row DELETE batches over the terminal-state subset only.

#### 14.6.2 RetentionRunner (opt-in policies — PI-scoped + engine-audit-scoped)

A `RetentionRunner` GenServer in `peripheral_persistence` starts at boot **only if at least one `EVIL_RETENTION_*_DAYS` env var is set** (§14.3 — either a PI-scoped `EVIL_RETENTION_<STATE>_DAYS` or the engine-audit `EVIL_RETENTION_ENGINE_AUDIT_DAYS`, or any combination). When started, each tick runs two independent passes:

**Pass A — PI-scoped (unchanged).** Sleeps for `EVIL_RETENTION_RUN_INTERVAL` (default `PT1H`). For each configured terminal-state PI policy, computes `cutoff = now - N_days` and selects up to `EVIL_RETENTION_BATCH_SIZE` (default `500`) eligible PIs ordered by `finished_at ASC`. Per eligible PI, executes a single transaction that deletes in this order: `process_instance_events` rows → `data_object_writes` rows → `data_objects` snapshot rows → `flow_node_instances` rows → `engine_timers` rows scoped to the eligible PI → `gateway_pending_arrivals` rows scoped to the eligible PI → the `process_instances` row itself. If any child PI (spawned via Call Activity) is still `running`, the parent is **skipped** (not deleted) — no orphan children. Emits one `Event.RetentionPurged{process_instance_id, purged_at, row_counts, policy_source: :retention_runner}` per deleted PI on `EngineEventBus`.

**Pass B — engine-audit-scoped.** If `EVIL_RETENTION_ENGINE_AUDIT_DAYS` is set, runs after Pass A with `cutoff = now - N_days` and sweeps, in order: `messages WHERE published_at < cutoff` → `pending_messages WHERE state IN ('delivered','expired','cancelled') AND published_at < cutoff` → `signals WHERE published_at < cutoff` → `pending_signals WHERE state IN ('delivered','expired','cancelled') AND published_at < cutoff` → `escalations WHERE published_at < cutoff` → `pending_escalations WHERE state IN ('delivered','expired','cancelled') AND published_at < cutoff` → `compensations WHERE triggered_at < cutoff` → `engine_timers WHERE state IN ('fired','cancelled') AND fire_at < cutoff`. Deletes are batched by `EVIL_RETENTION_BATCH_SIZE` per table (one transaction per batch), and **partition-aware** for the eight partitioned tables — PostgreSQL's partition pruning makes each batch touch at most one or two old partitions. Emits one `Event.EngineAuditPurged{table, cutoff, row_count, policy_source: :retention_runner}` per table on `EngineEventBus` so audit-sink plugins can record a per-table purge trail. Pass B does **not** touch `pending_messages.state='pending'`, `pending_signals.state='pending'`, `pending_escalations.state='pending'` (all operational live state, [routing.md](./routing.md) §3.5.4 / [routing.md](./routing.md) §3.5.6 / [routing.md](./routing.md) §3.5.7.1), or `engine_timers.state='armed'` rows (live wakes) — the WHERE clauses explicitly exclude them. Pass B is independent of Pass A: an operator who sets only `EVIL_RETENTION_ENGINE_AUDIT_DAYS` (and no PI-scoped policy) still gets engine-audit retention; the runner simply skips Pass A.
...
- Operational-state rows (`pending_messages.state='pending'`, `pending_signals.state='pending'`, `pending_escalations.state='pending'`, `engine_timers.state='armed'`) are live execution state, **never** retention-eligible regardless of how aggressive `EVIL_RETENTION_ENGINE_AUDIT_DAYS` is set.

#### 14.6.3 Manual purge (operator-driven)

Exposed as `Mutation.purgeProcessInstances` ([api.md](./api.md) §10.2.3) and as the CLI equivalent `evil_engine purge` shipped inside the release. The CLI is the recommended path for automation (scheduled sweeps outside the runner cadence, one-off compliance-driven deletions, pre-upgrade cleanup); the GraphQL mutation is the recommended path for in-tool use (admin dashboards).

`dryRun: true` is the default for the GraphQL mutation and the CLI's default mode. Operators must explicitly pass `dryRun: false` / `--no-dry-run` to actually delete. The response always returns the row counts that were (or would have been) deleted, so the operator can size up the cost before committing.
...
| **Studio engine-debugger views** | The **BPMN-flow view** (PI progress, FNI detail, sender↔receiver navigation, DO history, message/signal/escalation delivery traces, timer fires) reads always-on kernel tables — `process_instances` / `flow_node_instances` (with `triggerer_flow_node_instance_id`), `data_objects` / `data_object_writes`, `messages` / `signals` / `escalations` (with `correlations[]`), `engine_timers` — so it is unaffected by event sink configuration; PI retention trims its reach for `finished` PIs (e.g. `finished=30d` means the flow view reaches 30 days back for that state), and `EVIL_RETENTION_ENGINE_AUDIT_DAYS` caps how far back the message/signal/escalation delivery panels reach. A separate **flat "engine event log" panel** — if Studio chooses to implement one — would require a plugin event sink writing to a custom table; its reach would be capped by PI retention the same way (events purge cascades with the parent PI). |
| **Resume** | Irrelevant — retention only touches terminal PIs + terminal-state engine-audit rows. A `running` PI in a partition older than the retention cutoff is still resumable. An `armed` timer or `pending` pending-message is always preserved regardless of age. |
...
| **Monthly partitions** | Retention runner's `DELETE` is partition-aware across all nine partitioned tables (`process_instance_events`, `data_object_writes`, `messages`, `pending_messages`, `signals`, `pending_signals`, `escalations`, `pending_escalations`, `compensations`): rows land in the right partition automatically, and future v2 archival can `DETACH` + `DROP` whole old partitions for a given month. |
...
| **`EVIL_PENDING_ESCALATIONS_KEEP_AFTER_TRANSITION=false`** | Identical to above, applied to `pending_escalations`. Late-catch audit vanishes on transition; `escalations.outcome='late_caught_observed'` / `'uncaught_root'` itself is still recorded in `escalations` and subject to normal engine-audit retention. |
...
- Per-table retention granularity for engine-audit tables: one knob covers all engine-audit tables (`messages`, `pending_messages`, `signals`, `pending_signals`, `escalations`, `pending_escalations`, `compensations`, and the terminal-state rows of `engine_timers`). Operators who need "keep messages for 1y, signals for 30d" run an external archival sink plugin.
- Manual `purgeEngineAudit` GraphQL mutation: the PI-scoped `purgeProcessInstances` mutation has no engine-audit equivalent in v1. Operators who need ad-hoc engine-audit cleanup set `EVIL_RETENTION_ENGINE_AUDIT_DAYS` temporarily low, or run direct SQL under the admin DB role.
