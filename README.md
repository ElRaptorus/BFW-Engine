...
| **Load tests**        | `mix test.load`        | 15 benchmarks in two suites: **Resume** — resume 100–10,000 PIs across varying process types and FNI counts, plus seeding throughput at 1K/5K/10K batch sizes. **Execution** - full API-driven lifecycle: deploy via HTTP, start 100–10,000 PIs through the API across 5 fixture types (linear, chained tasks, sync/async service tasks, user tasks), with auto-finishing of user tasks via an EngineEventBus sink. |
