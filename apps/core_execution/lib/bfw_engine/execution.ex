defmodule BfwEngine.Execution do
  @moduledoc """
  ...
  **before** the supervisor is touched.
  ...
  def count_active do
    %{active: count} = DynamicSupervisor.count_children(BfwEngine.Execution.Supervisor)
    count
  end
  ...
  """
  @spec abort_process_instance(String.t(), String.t() | nil, BfwEngine.Types.Identity.t() | nil) ::
          :ok | {:error, term()}
  ...
  defp lookup_process_instance_for_flow_node_instance(flow_node_instance_id) do
    case Registry.lookup(BfwEngine.Execution.Registry, {:fni, flow_node_instance_id}) do
      [{pid, _}] -> {:ok, pid}
  ...
  defp running_process_instance_pids do
    case Process.whereis(BfwEngine.Execution.Supervisor) do
      nil ->
        []
  ...
  def lookup_process_instance(process_instance_id) do
    case Registry.lookup(BfwEngine.Execution.Registry, process_instance_id) do
      [{pid, _}] -> {:ok, pid}
      [] -> {:error, :not_found}
    end
  end
  ...
  Called by `BfwEngine.Api.retry_process_instance/3` after the targeted PI
  ...
  defp emit_retry_event(
    ...
    :telemetry.execute(
      [:bfw_engine, :process_instance, :retried],
