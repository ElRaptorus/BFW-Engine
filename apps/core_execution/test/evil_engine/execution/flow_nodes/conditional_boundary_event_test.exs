defmodule EvilEngine.Execution.FlowNodes.ConditionalBoundaryEventTest do
  use ExUnit.Case, async: false

  alias EvilEngine.BPMN.Model.EventDefinition
  alias EvilEngine.BPMN.Model.FlowNode
  alias EvilEngine.BPMN.Model.FlowNodeData
  alias EvilEngine.Execution.FlowNodes.ConditionalBoundaryEvent
  ...
  describe "handle_enter/3 — condition false, parks as waiting" do
    test "returns {:wait, ...} when condition is false" do
      ...
      assert {:wait, %EvilEngine.Execution.FlowNodeResult{} = result} =
               ConditionalBoundaryEvent.handle_enter(flow_node, token, context)
