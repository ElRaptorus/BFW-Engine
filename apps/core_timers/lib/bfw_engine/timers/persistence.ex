defmodule BfwEngine.Timers.Persistence do
  @moduledoc """
  ...
  The production default is `BfwEngine.Persistence.TimerStartScheduleAdapter`
  (`config/config.exs`). Cycle Timer Start schedules survive process
  restart via the `timer_start_schedules` table.
  For unit tests in `core_timers`, `BfwEngine.Timers.Persistence.NoOp`
  provides an in-memory stub (`config/test.exs`).
  """
