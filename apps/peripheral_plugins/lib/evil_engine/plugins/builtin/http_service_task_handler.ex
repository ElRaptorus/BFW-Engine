defmodule EvilEngine.Plugins.Builtin.HttpServiceTaskHandler do
  @moduledoc """
  Built-in HTTP Service Task handler.

  Performs asynchronous HTTP requests based on `evil:http*` extension
  ...
  `EvilEngine.Execution.finish_async_service_task/2` on success or
  `EvilEngine.Execution.fail_async_service_task/3` on failure.
  ...
  defp execute_and_complete(
    ...
    case execute_request(req_opts, url, type_data, flow_node, token, handler_context) do
      {:ok, output} ->
        EvilEngine.Execution.finish_async_service_task(flow_node_instance_id, output)

      {:error, reason} ->
        EvilEngine.Execution.fail_async_service_task(
          flow_node_instance_id,
          error_code(reason),
          inspect(reason)
        )
  ...
  defp evaluate_feel(expression, flow_node, token, handler_context, error_key) do
    ...
    case EvilEngine.Expressions.eval(expression, context) do
      {:ok, result} -> {:ok, result}
