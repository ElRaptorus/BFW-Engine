defmodule BfwEngine.Plugins.LoaderTest do
  ...
  test "registers built-in http service task handler on startup" do
    ...
    assert Enum.any?(handlers, fn capability ->
             capability.descriptor[:implementation] == "http" &&
               ...
               capability.plugin_name == "bfw:builtin"
           end)
  end
  ...
  test "facade_for_plugin/1 builds EngineFacade with required fields and wiring" do
    ...
    refute Map.has_key?(facade, :register_persistence_adapter)
    refute Map.has_key?(facade, :register_monitoring_panel)
    refute Map.has_key?(facade, :register_timer_source)
    refute Map.has_key?(facade, :register_data_store_adapter)
