defmodule EvilEngine.SDK.BPMN do
  @moduledoc """
  ...
  `EvilEngine.BPMN.Model.*` struct — `Definitions`, `Process`, `FlowNode`,
  ...
  or Hex consumer of `evil_engine_sdk` under its real `EvilEngine.BPMN.*`
  name; there is nothing to alias.

  This module exists to give the **functions** — parsing and cache lookup
  — a documented, discoverable entry point under the `EvilEngine.SDK`
  namespace, mirroring the read-only projection the GraphQL Model graph
  (`EvilEngineWeb.Graphql.ModelResolvers`) exposes over the wire. Same
  semantics, same source of truth (`ModelCache`), no HTTP round-trip.

  ## Parsing undeployed XML

      {:ok, %EvilEngine.BPMN.Model.Definitions{} = definitions} =
        EvilEngine.SDK.BPMN.parse(xml)

  ## Reading a deployed model

      {:ok, %EvilEngine.BPMN.Model.Definitions{} = definitions} =
        EvilEngine.SDK.BPMN.fetch_definitions(process_version_id)

  `fetch_definitions/1` reads through `EvilEngine.BPMN.ModelCache`: a warm
  ...
  @doc """
  Parses BPMN 2.0 XML into a `%EvilEngine.BPMN.Model.Definitions{}` tree.
