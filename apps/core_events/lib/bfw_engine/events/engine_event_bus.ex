defmodule BfwEngine.Events.EngineEventBus do
  @moduledoc """
  Single typed fan-out surface for every `BfwEngine.Types.Event.*`.
  ...
  `BfwEngine.Events.SinkWorker` GenServers, supervised by
  `BfwEngine.Events.SinkSupervisor`.
  ...
  alias BfwEngine.Events.SinkWorker
  ...
  def handle_call({:register_sink, name, module, opts}, _from, state) do
    if Map.has_key?(state.workers, name) do
      ...
      case DynamicSupervisor.start_child(BfwEngine.Events.SinkSupervisor, child_spec) do
        {:ok, _pid} ->
