defmodule BfwEngine.Events.EngineEventBusTest do
  ...
  describe "register_sink/3" do
    test "registers a sink and lists it" do
      assert :ok =
               EngineEventBus.register_sink("test-sink", BfwEngine.Test.TestSink,
                 test_pid: self()
               )
      ...
      assert hd(sinks).module == BfwEngine.Test.TestSink
    end
    ...
    test "returns :already_registered when name is registered twice" do
      assert :ok =
               EngineEventBus.register_sink("dup-name", BfwEngine.Test.TestSink, test_pid: self())
  ...
  describe "publish/1" do
    ...
    test "dispatches event to registered sink" do
      :ok =
        EngineEventBus.register_sink("dispatch-test", BfwEngine.Test.TestSink, test_pid: self())
    ...
    test "dispatches to multiple sinks" do
      :ok = EngineEventBus.register_sink("multi-a", BfwEngine.Test.TestSink, test_pid: self())
      :ok = EngineEventBus.register_sink("multi-b", BfwEngine.Test.TestSink, test_pid: self())
    ...
    test "selective sink only receives accepted events" do
      :ok =
        EngineEventBus.register_sink("selective", BfwEngine.Test.SelectiveSink, test_pid: self())
  ...
  describe "publish/1 with :skip return" do
    test "sink returning :skip preserves its previous state" do
      :ok =
        EngineEventBus.register_sink("skipper", BfwEngine.Test.SkippingSink, test_pid: self())
  ...
  describe "shutdown_sinks/0" do
    test "calls handle_shutdown on registered sinks" do
      :ok =
        EngineEventBus.register_sink("shutdown-test", BfwEngine.Test.TestSink, test_pid: self())
  ...
  describe "shutdown_sinks/0 with crashing sink" do
    ...
    test "handles dead worker gracefully" do
      :ok = EngineEventBus.register_sink("to-kill", BfwEngine.Test.TestSink, test_pid: self())

      [{worker_pid, _}] = Registry.lookup(BfwEngine.Events.SinkRegistry, "to-kill")
  ...
  describe "reset_state/0" do
    test "unregistering via reset_state stops further delivery" do
      :ok =
        EngineEventBus.register_sink("unsub-test", BfwEngine.Test.TestSink, test_pid: self())
    ...
    test "tolerates already-dead worker during reset" do
      :ok =
        EngineEventBus.register_sink("dead-on-reset", BfwEngine.Test.TestSink, test_pid: self())

      [{worker_pid, _}] = Registry.lookup(BfwEngine.Events.SinkRegistry, "dead-on-reset")
      DynamicSupervisor.terminate_child(BfwEngine.Events.SinkSupervisor, worker_pid)
      Process.sleep(20)
  ...
  describe "crash isolation" do
    test "crashing sink does not kill the bus" do
      :ok = EngineEventBus.register_sink("crasher", BfwEngine.Test.CrashingSink, [])
      :ok = EngineEventBus.register_sink("survivor", BfwEngine.Test.TestSink, test_pid: self())
    ...
    test "crashing sink emits SinkFailed event to surviving sinks" do
      :ok = EngineEventBus.register_sink("crasher2", BfwEngine.Test.CrashingSink, [])
      :ok = EngineEventBus.register_sink("observer", BfwEngine.Test.TestSink, test_pid: self())
  ...
  describe "PF-3: per-sink worker isolation" do
    ...
    test "a slow sink does not block other sinks" do
      ...
      :ok = EngineEventBus.register_sink("fast", BfwEngine.Test.TestSink, test_pid: self())
    ...
    test "supervisor restarts a worker after the worker process is killed" do
      :ok =
        EngineEventBus.register_sink("restartable", BfwEngine.Test.TestSink, test_pid: self())

      [{first_pid, _}] = Registry.lookup(BfwEngine.Events.SinkRegistry, "restartable")
