defmodule BfwEngine.Auth.Plug do
  @moduledoc """
  Plug pipeline that extracts and verifies JWT tokens from the
  `Authorization: Bearer <token>` header.

  When `BFE_AUTH_DISABLED=true`, injects a synthetic anonymous
  `%BfwEngine.Types.Identity{}` with least-privilege defaults.

  ## Conn assigns

  On success, sets:
  - `conn.assigns.identity` — `%BfwEngine.Types.Identity{}`
