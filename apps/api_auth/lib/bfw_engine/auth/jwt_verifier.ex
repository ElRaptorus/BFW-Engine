defmodule BfwEngine.Auth.JwtVerifier do
  @moduledoc """
  ...
  When `BFE_JWT_ISSUER` / `BFE_JWT_AUDIENCE` are configured, the
  ...
  - `{:error, :issuer_mismatch}` — `BFE_JWT_ISSUER` configured and `iss` does not match
  - `{:error, :audience_mismatch}` — `BFE_JWT_AUDIENCE` configured and `aud` does not match
  """
