defmodule BfwEngine.Telemetry.MeasurementsTest do
  ...
  describe "active_process_instances/0" do
    test "emits telemetry event with active PI count" do
      ...
      :telemetry.attach(
        handler_id,
        [:bfw_engine, :process_instance, :active],
    ...
    test "emits capacity ratio gauge" do
      ...
      :telemetry.attach(
        handler_id,
        [:bfw_engine, :process_instance, :capacity],
        fn _event, measurements, _metadata, _config ->
  ...
  describe "overload threshold crossing" do
    ...
    test "no overload event when cap is infinity" do
      ...
      assert :persistent_term.get(:bfw_engine_load_level, :normal) == :normal
    end
  ...
  describe "level transitions" do
    ...
    test "M-1: transition from elevated to normal updates persistent_term" do
      ...
      :persistent_term.put(:bfw_engine_load_level, :elevated)

      Measurements.active_process_instances()

      assert :persistent_term.get(:bfw_engine_load_level) == :normal
    end

    test "M-2: transition from critical to normal updates persistent_term" do
      Application.put_env(:core_execution, :max_concurrent_process_instances, 1000)
      :persistent_term.put(:bfw_engine_load_level, :critical)
  ...
  describe "upward threshold crossings" do
    ...
    test "normal to elevated publishes EngineOverloaded at 70% capacity" do
      ...
      actual_count = DynamicSupervisor.count_children(BfwEngine.Execution.Supervisor).active
      ...
      assert :persistent_term.get(:bfw_engine_load_level) == :elevated
    end
    ...
    test "elevated to critical publishes EngineOverloaded at 90% capacity" do
      ...
      assert :persistent_term.get(:bfw_engine_load_level) == :critical
    end
  ...
  defp cleanup_all_dummy_children do
    ...
    Enum.each(child_pids, fn child_pid ->
      try do
        DynamicSupervisor.terminate_child(BfwEngine.Execution.Supervisor, child_pid)
      catch
        :exit, _ -> :ok
      end
    end)
      ...
      for _index <- 1..count do
        {:ok, child_pid} =
          DynamicSupervisor.start_child(
            BfwEngine.Execution.Supervisor,
            {Agent, fn -> :ok end}
          )
