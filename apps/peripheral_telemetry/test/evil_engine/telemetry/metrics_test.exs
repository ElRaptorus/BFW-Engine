defmodule EvilEngine.Telemetry.MetricsTest do
  ...
  describe "metrics/0" do
    ...
    test "includes all expected metric names" do
      ...
      expected = [
        [:evil_engine, :http, :request, :total],
        [:evil_engine, :http, :request, :duration_ms],
        [:evil_engine, :process_instance, :state_change, :total],
        [:evil_engine, :process_instance, :active, :count],
        [:evil_engine, :flow_node_instance, :started, :total],
        [:evil_engine, :flow_node_instance, :state_change, :total],
        [:evil_engine, :event_bus, :events, :total],
        [:evil_engine, :process_instance, :capacity, :ratio],
        [:evil_engine, :dmn, :evaluations, :total],
        [:evil_engine, :dmn, :evaluate, :duration, :milliseconds],
        [:evil_engine, :dmn, :evaluations, :exceptions, :total],
        [:evil_engine, :dmn, :cache, :hit, :total],
        [:evil_engine, :dmn, :cache, :miss, :total],
        [:evil_engine, :db, :query, :queue_time_ms],
        [:evil_engine, :db, :query, :total_time_ms],
        [:evil_engine, :db, :query, :count],
        [:evil_engine, :db, :pool, :size],
        [:evil_engine, :db, :pool, :checked_out],
        [:evil_engine, :db, :pool, :idle],
        [:vm, :memory, :total],
        [:vm, :memory, :processes],
        ...
        [:evil_engine, :escalation, :raised, :total],
        [:evil_engine, :escalation, :uncaught, :total]
      ]
    ...
    test "HTTP request counter has method/route/status tags" do
      http_counter =
        Metrics.metrics()
        |> Enum.find(&(&1.name == [:evil_engine, :http, :request, :total]))
    ...
    test "FNI state_change counter has flow_node_type/terminal_state tags" do
      fni_counter =
        Metrics.metrics()
        |> Enum.find(&(&1.name == [:evil_engine, :flow_node_instance, :state_change, :total]))
    ...
    test "event bus counter has event_type tag" do
      bus_counter =
        Metrics.metrics()
        |> Enum.find(&(&1.name == [:evil_engine, :event_bus, :events, :total]))
    ...
    test "DMN evaluations counter has hit_policy tag" do
      dmn_counter =
        Metrics.metrics()
        |> Enum.find(&(&1.name == [:evil_engine, :dmn, :evaluations, :total]))
    ...
    test "DMN evaluate duration is a distribution with millisecond buckets" do
      dmn_duration =
        Metrics.metrics()
        |> Enum.find(&(&1.name == [:evil_engine, :dmn, :evaluate, :duration, :milliseconds]))
    ...
    test "HTTP duration is a distribution with millisecond buckets" do
      duration =
        Metrics.metrics()
        |> Enum.find(&(&1.name == [:evil_engine, :http, :request, :duration_ms]))
