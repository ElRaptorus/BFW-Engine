defmodule EvilEngine.Telemetry.StatsCollector do
  @moduledoc """
  ...
  @timer_primary_table :evil_engine_timers_primary
  ...
  defp query_plugins do
    EvilEngine.Plugins.Registry.list_plugins()
  rescue
    _ -> []
  catch
    :exit, _ -> []
  ...
  defp compute_load_level(limit) when is_integer(limit) and limit > 0 do
    active =
      try do
        %{active: n} = DynamicSupervisor.count_children(EvilEngine.Execution.Supervisor)
        n
      catch
        :exit, _ -> 0
      end
  ...
  defp started_at do
    case :persistent_term.get(:evil_engine_started_at, nil) do
      nil ->
        now = DateTime.utc_now()
        :persistent_term.put(:evil_engine_started_at, now)
        now
