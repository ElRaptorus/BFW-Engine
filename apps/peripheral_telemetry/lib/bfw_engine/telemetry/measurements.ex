defmodule BfwEngine.Telemetry.Measurements do
  @moduledoc """
  ...
  @execution_supervisor :"Elixir.BfwEngine.Execution.Supervisor"
  ...
  def active_process_instances do
    ...
    :telemetry.execute([:bfw_engine, :process_instance, :active], %{count: active_count}, %{})
  ...
  defp emit_capacity_ratio(_active, :infinity) do
    :telemetry.execute([:bfw_engine, :process_instance, :capacity], %{ratio: 0.0}, %{})
  end

  defp emit_capacity_ratio(active, limit) when is_integer(limit) and limit > 0 do
    ratio = active / limit
    :telemetry.execute([:bfw_engine, :process_instance, :capacity], %{ratio: ratio}, %{})
  end
  ...
  defp check_overload_crossing(active, limit) when is_integer(limit) and limit > 0 do
    ...
    previous = :persistent_term.get(:bfw_engine_load_level, :normal)

    if new_level != previous do
      :persistent_term.put(:bfw_engine_load_level, new_level)
