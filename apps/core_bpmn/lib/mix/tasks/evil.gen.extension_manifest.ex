defmodule Mix.Tasks.Evil.Gen.ExtensionManifest do
  @moduledoc """
  Generate `extension-manifest.json` from `EvilEngine.BPMN.ExtensionManifest`
  ...
  ## Usage
      ...
      mix evil.gen.extension_manifest
      mix evil.gen.extension_manifest --check
  ...
  Writes two identical copies, both intended to be committed:
    ...
    * `packages/js/sdk/src/generated/extension-manifest.json` (shipped in
      `@elraptorus/daemonengine_sdk`, the contract-layer package, per D-7)
  """
  ...
  defp check do
    ...
    if drifted == [] do
      ...
      Mix.shell().error(
        "extension-manifest.json is out of date. Run `mix evil.gen.extension_manifest` and commit the result. Drifted paths:\n" <>
