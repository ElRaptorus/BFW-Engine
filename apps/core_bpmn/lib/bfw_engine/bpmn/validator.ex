defmodule BfwEngine.BPMN.Validator do
  @moduledoc """
  ...
  - Definitions-level extension namespace (canonical `xmlns:bfw` required; legacy `xmlns:evil` and URIs rejected)
  """
  ...
  @canonical_extension_namespace_uri "https://bifrostforge.world/schema/bpmn"
  @canonical_extension_namespace_prefix "bfw"
  @canonical_extension_namespace_declaration "xmlns:#{@canonical_extension_namespace_prefix}=\"#{@canonical_extension_namespace_uri}\""

  @forbidden_legacy_extension_namespace_prefix "xmlns:evil"

  @forbidden_legacy_extension_namespace_uris [
    "https://evilengine.dev/schema/bpmn",
    "http://evilengine.dev/bpmn",
    "http://evilengine.io/schema/bpmn",
    "https://evil.studio/schema/bpmn/platform/1.0"
  ]
  ...
  def validate(%Definitions{} = definitions) do
    violations =
      check_extension_namespace(definitions) ++
        Enum.flat_map(definitions.processes, &validate_process(&1, definitions))
  ...
  defp check_extension_namespace(%Definitions{} = definitions) do
    check_legacy_extension_namespace(definitions.raw_xml) ++
      check_missing_extension_namespace(definitions)
  end

  defp check_legacy_extension_namespace(raw_xml) do
    prefix_violations =
      if String.contains?(raw_xml, @forbidden_legacy_extension_namespace_prefix) do
        [
          {:legacy_extension_namespace,
           "BPMN definitions declare the forbidden extension prefix " <>
             "'#{@forbidden_legacy_extension_namespace_prefix}'. " <>
             "Declare #{@canonical_extension_namespace_declaration} instead."}
        ]
      else
        []
      end
    ...
    uri_violations =
      @forbidden_legacy_extension_namespace_uris
      |> Enum.filter(&String.contains?(raw_xml, &1))
      |> Enum.map(fn forbidden_uri ->
        {:legacy_extension_namespace,
         "BPMN definitions declare the forbidden extension namespace URI " <>
           "'#{forbidden_uri}'. Declare #{@canonical_extension_namespace_declaration} instead."}
      end)

    prefix_violations ++ uri_violations
  end

  defp check_missing_extension_namespace(%Definitions{raw_xml: raw_xml, processes: processes}) do
    has_executable_process = Enum.any?(processes, & &1.is_executable)

    if has_executable_process and
         not String.contains?(raw_xml, @canonical_extension_namespace_declaration) do
      [
        {:missing_extension_namespace,
         "BPMN definitions with at least one executable process must declare " <>
           "#{@canonical_extension_namespace_declaration}."}
      ]
    else
      []
    end
  ...
  defp check_version(%BpmnProcess{version: nil, id: id}) do
    [
      {:missing_version, "Process '#{id}' is missing required property: version (bfw:version)"}
    ]
  end
  ...
  defp check_version(%BpmnProcess{version: "", id: id}) do
    [
      {:blank_version,
       "Process '#{id}' has a blank value for required property: version (bfw:version)"}
    ]
  end
    ...
    Enum.flat_map(process.data_objects, fn obj ->
      case obj.value_contract do
            ...
            _ ->
              [
                {:invalid_value_contract,
                 "DataObject '#{obj.id}' has an invalid bfw:valueContract JSON Schema"}
              ]
          end
        ...
        _ ->
          [
            {:invalid_value_contract,
             "DataObject '#{obj.id}' has an invalid bfw:valueContract (must be a JSON object)"}
          ]
      end
    end)
  end
  ...
  # `BfwEngine.Execution.ProcessInstance.resolve_start_event/2`): an inner Start
      ...
      [
        {:incomplete_flow_node,
         scope_label <>
           ...
           "required property: decisionRef (bfw:decisionRef)"}
      ]
    else
      []
    end
    ...
    [
      {:adhoc_sequential_missing_active_elements,
       "AdHocSubProcess '#{subprocess_id}' has sequential ordering without an implementation " <>
         "or bfw:ActiveElements expression. The engine has no deterministic basis for " <>
      ...
      [
        {:mi_missing_collection,
         "#{type_label(type)} '#{id}' has multiInstanceLoopCharacteristics but no resolvable " <>
           "collection (bfw:inputCollection or loopDataInput is required)"}
      ]
    else
      []
    end
      ...
      n when is_integer(n) and n <= 0 ->
        [
          {:mi_invalid_max_iterations,
           "#{type_label(type)} '#{id}' has bfw:maxIterations=#{n}; must be > 0"}
        ]
    ...
    if is_binary(loop_break_condition) and blank?(loop_break_condition) do
      [
        {:mi_blank_break_condition,
         "#{type_label(type)} '#{id}' has an empty bfw:loopBreakCondition; " <>
           "remove it or provide a valid FEEL expression"}
      ]
    else
      []
      ...
      [
        {:loop_cardinality_not_supported,
         "#{type_label(type)} '#{id}' declares loopCardinality, which is not supported; " <>
           "use bfw:inputCollection (iteration count is the collection length)"}
      ]
    else
      []
    end
