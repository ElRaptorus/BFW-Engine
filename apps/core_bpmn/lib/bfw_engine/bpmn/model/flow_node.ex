defmodule BfwEngine.BPMN.Model.FlowNode do
  @moduledoc """
...
defmodule BfwEngine.BPMN.Model.FlowNodeData.Task do
  @moduledoc "Untyped `<bpmn:task>` — pure pass-through."
  @type t :: %__MODULE__{}
  defstruct []
end

defmodule BfwEngine.BPMN.Model.FlowNodeData.UserTask do
  @moduledoc """
  `<bpmn:userTask>` with `bfw:*` extensions for form fields,
...
defmodule BfwEngine.BPMN.Model.FlowNodeData.ServiceTask do
  @moduledoc """
...
defmodule BfwEngine.BPMN.Model.FlowNodeData.ScriptTask do
  @moduledoc """
  ...
  `bfw:scriptRef` is a custom extension element for plugin dispatch.
...
defmodule BfwEngine.BPMN.Model.FlowNodeData.BusinessRuleTask do
  @moduledoc """
  ...
  - `"dmn"` — Resolve and evaluate a deployed DMN decision table via `bfw:decisionRef`.
    When the DMN model contains multiple decisions, `bfw:decisionElementId`
  ...
  All `bfw:*` fields are engine-specific extensions.
  """
...
defmodule BfwEngine.BPMN.Model.FlowNodeData.SendTask do
  @moduledoc """
...
defmodule BfwEngine.BPMN.Model.FlowNodeData.ReceiveTask do
  @moduledoc """
...
defmodule BfwEngine.BPMN.Model.FlowNodeData.CallActivity do
  @moduledoc """
  ...
  `start_event_id` (from `<bfw:startEventId>`) selects which Start Event
  the child process should begin at. Required when the child has multiple
  untyped Start Events; optional otherwise.

  `called_process_version` (from `<bfw:calledProcessVersion>`) pins the
  child to a specific `bfw:version` string. `nil` or blank means the
...
defmodule BfwEngine.BPMN.Model.FlowNodeData.SubProcess do
  @moduledoc """
...
defmodule BfwEngine.BPMN.Model.FlowNodeData.ExclusiveGateway do
...
defmodule BfwEngine.BPMN.Model.FlowNodeData.ParallelGateway do
  @moduledoc "`<bpmn:parallelGateway>` — AND split/join."
  @type t :: %__MODULE__{}
  defstruct []
end

defmodule BfwEngine.BPMN.Model.FlowNodeData.InclusiveGateway do
...
defmodule BfwEngine.BPMN.Model.FlowNodeData.EventBasedGateway do
  @moduledoc "`<bpmn:eventBasedGateway>` — first catch wins, others cancelled."
  @type t :: %__MODULE__{}
  defstruct []
end

defmodule BfwEngine.BPMN.Model.FlowNodeData.ComplexGateway do
...
defmodule BfwEngine.BPMN.Model.FlowNodeData.Unknown do
