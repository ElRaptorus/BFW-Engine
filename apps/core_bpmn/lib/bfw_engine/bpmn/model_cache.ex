defmodule BfwEngine.BPMN.ModelCache do
  @moduledoc """
  ...
  **waiters** in the GenServer state and replied to when the load
  ...
  @table :bfw_engine_model_cache
  @message_start_index :bfw_engine_message_start_index
  @signal_start_index :bfw_engine_signal_start_index
  ...
  Emits `[:bfw_engine, :model_cache, :fetch]` on every call (hit or
      ...
      {mod, fun} ->
        case apply(mod, fun, [id]) do
          {:ok, xml} when is_binary(xml) ->
            BfwEngine.BPMN.parse_and_validate(xml)
