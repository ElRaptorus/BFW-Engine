defmodule BfwEngine.BPMN.ExtensionManifestTest do
  @moduledoc """
  Tests for `BfwEngine.BPMN.ExtensionManifest` (WP-5, WP-7).

  Two guarantees:

  1. **Corpus round-trip (WP-5.4):** every `bfw:*` element name that
     ...
     a new `bfw:*` element without a manifest entry fails this test.
  ...
  describe "corpus round-trip (WP-5.4)" do
    test "every bfw:* element name found in the fixture corpus is a known manifest entry or extensible container" do
      ...
      corpus_elements = corpus_bfw_elements()

      assert corpus_elements != MapSet.new(),
             "fixture corpus scan found zero bfw:* elements — check fixture paths"

      unknown = MapSet.difference(corpus_elements, known)

      assert MapSet.size(unknown) == 0,
             "bfw:* elements in the fixture corpus with no manifest entry: #{Enum.join(unknown, ", ")}"
    end
  end
      ...
      for relative_path <- [
        ...
        assert actual == expected,
               "#{relative_path} is out of date (#{Path.join(@repo_root, relative_path)}). " <>
                 "Run `mix bfw.gen.extension_manifest` and commit the result."
      end
    end
  end

  defp corpus_bfw_elements do
    [@root_fixtures, @core_fixtures]
    |> Enum.flat_map(&walk_bpmn_files/1)
    |> Enum.flat_map(fn path ->
      path
      |> File.read!()
      |> then(&Regex.scan(~r/<\/?bfw:([A-Za-z][A-Za-z0-9]*)/, &1))
