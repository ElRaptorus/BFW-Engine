defmodule BfwEngine.BPMN.ValidatorTest do
  ...
  alias BfwEngine.BPMN.Validator

  @fixtures_dir Path.join([__DIR__, "..", "..", "fixtures", "bpmns"])

  @canonical_extension_namespace_declaration ~s(xmlns:bfw="https://bifrostforge.world/schema/bpmn")
  ...
  defp minimal_valid_definitions(overrides) do
    ...
    %Definitions{
      raw_xml: @canonical_extension_namespace_declaration,
      processes: [struct!(BpmnProcess, proc_fields)]
    }
  end
  ...
  describe "validate/1 — missing version" do
    test "rejects process without bfw:version" do
  ...
  # -------------------------------------------------------------------------
  # Definitions-level extension namespace
  # -------------------------------------------------------------------------

  describe "validate/1 — extension namespace" do
    test "rejects xmlns:evil together with a legacy URI" do
      xml = """
      <?xml version="1.0" encoding="UTF-8"?>
      <bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL"
                        xmlns:evil="https://evilengine.dev/schema/bpmn"
                        id="Definitions_1">
        <bpmn:process id="Process_1" name="Legacy Namespace" isExecutable="true">
      ...
      assert {:error, violations} = Validator.validate(definitions)
      assert Enum.any?(violations, fn {code, _} -> code == :legacy_extension_namespace end)

      {_code, message} =
        Enum.find(violations, fn {code, _} -> code == :legacy_extension_namespace end)

      assert message =~ "xmlns:evil" or message =~ "https://evilengine.dev/schema/bpmn"
    end

    test "rejects executable process that omits xmlns:bfw canonical URI" do
      xml = """
      <?xml version="1.0" encoding="UTF-8"?>
      <bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL"
                        id="Definitions_1">
        <bpmn:process id="Process_1" name="Missing Namespace" isExecutable="true">
          <bpmn:extensionElements>
      ...
      definitions = parse_fixture_from_xml(xml)
      assert {:error, violations} = Validator.validate(definitions)
      assert Enum.any?(violations, fn {code, _} -> code == :missing_extension_namespace end)
    end

    test "accepts xmlns:bfw canonical URI with bfw:version" do
      definitions = parse_fixture("minimal_valid.bpmn")
      assert {:ok, ^definitions} = Validator.validate(definitions)
    end
  end
  ...
  describe "validate/1 — data association references" do
    test "dangling DOA target_ref produces error" do
      definitions =
        minimal_valid_definitions(
          process: [
            data_objects: [%BfwEngine.BPMN.Model.DataObject{id: "DO_1"}],
            data_object_references: [
              %BfwEngine.BPMN.Model.DataObjectReference{id: "DOR_1", data_object_ref: "DO_1"}
            ]
          ],
          extra_nodes: [
            %FlowNode{
          ...
          extra_nodes: [
            %FlowNode{
              ...
              data_input_associations: [
                %BfwEngine.BPMN.Model.DataAssociation{id: "DIA_bad", source_ref: "NONEXISTENT"}
              ]
            }
          ],
          extra_flows: [
        ...
        minimal_valid_definitions(
          ...
          extra_nodes: [
            %FlowNode{
              ...
              type_data: %FlowNodeData.ScriptTask{script: "1+1"},
              data_output_associations: [
                %BfwEngine.BPMN.Model.DataAssociation{id: "DOA_1", target_ref: "DOR_1"}
              ],
              data_input_associations: [
                %BfwEngine.BPMN.Model.DataAssociation{id: "DIA_1", source_ref: "DOR_1"}
              ]
            }
          ],
          extra_flows: [
    ...
    test "invalid JSON Schema on DataObject produces error" do
      ...
      definitions =
        minimal_valid_definitions(
          process: [
            data_objects: [
              %BfwEngine.BPMN.Model.DataObject{id: "DO_1", value_contract: broken_schema}
            ],
            data_object_references: []
          ]
        )
    ...
    test "non-map value_contract produces error" do
      definitions =
        minimal_valid_definitions(
          process: [
            data_objects: [
              %BfwEngine.BPMN.Model.DataObject{id: "DO_1", value_contract: "not a map"}
            ],
            data_object_references: []
          ]
        )
    ...
    test "valid JSON Schema on DataObject passes" do
      definitions =
        minimal_valid_definitions(
          process: [
            data_objects: [
              %BfwEngine.BPMN.Model.DataObject{
                id: "DO_1",
    ...
    test "nil value_contract passes without error" do
      definitions =
        minimal_valid_definitions(
          process: [
            data_objects: [
              %BfwEngine.BPMN.Model.DataObject{id: "DO_1", value_contract: nil}
            ],
            data_object_references: []
          ]
        )
