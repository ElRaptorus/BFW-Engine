defmodule EvilEngine.DMN.ModelCacheTelemetryTest do
  @moduledoc false
  ...
  describe "fetch/1 cache telemetry" do
    test "emits :hit event when entry is already cached" do
      ...
      assert_received {:telemetry_event, [:evil_engine, :dmn, :cache, :hit], %{count: 1},
    ...
    test "emits :miss event on empty cache before load attempt" do
      ...
      assert_received {:telemetry_event, [:evil_engine, :dmn, :cache, :miss], %{count: 1},
                       %{decision_version_id: ^decision_version_id}}
    end
  end
end
