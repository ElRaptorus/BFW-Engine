defmodule BfwEngine.DMN.EvaluatorTelemetryTest do
  @moduledoc false
  ...
  defp await_telemetry_event(event_suffix, predicate, timeout \\ 500) do
    receive do
      {:telemetry_event, [:bfw_engine, :dmn, :evaluate, ^event_suffix], measurements, metadata} ->
  ...
  setup do
    ...
    :telemetry.attach_many(
      handler_id,
      [
        [:bfw_engine, :dmn, :evaluate, :start],
        [:bfw_engine, :dmn, :evaluate, :stop],
        [:bfw_engine, :dmn, :evaluate, :exception]
      ],
