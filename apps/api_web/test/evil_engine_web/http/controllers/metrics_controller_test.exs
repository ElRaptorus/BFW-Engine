defmodule EvilEngineWeb.Http.MetricsControllerTest do
  use ExUnit.Case, async: false

  import Plug.Test
  import Plug.Conn

  alias EvilEngineWeb.Http.MetricsController
  ...
  describe "index/2 when metrics disabled" do
    test "returns 404 with metrics_disabled error" do
      ...
      conn =
        conn(:get, "/metrics")
        |> put_private(:phoenix_format, "json")
        |> put_private(:phoenix_endpoint, EvilEngineWeb.Http.Endpoint)
        |> put_private(:phoenix_router, EvilEngineWeb.Http.Router)
        |> MetricsController.index(%{})
