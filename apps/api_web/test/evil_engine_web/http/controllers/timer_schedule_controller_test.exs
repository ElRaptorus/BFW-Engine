defmodule EvilEngineWeb.Http.TimerScheduleControllerTest do
  use ExUnit.Case, async: false

  import Plug.Test

  @endpoint EvilEngineWeb.Http.Endpoint
  @test_secret "test_only_secret_at_least_32_bytes!"

  defp call(conn), do: @endpoint.call(conn, @endpoint.init([]))

  defp sign_jwt(claims) do
    Application.put_env(:api_auth, :hs256_secret, @test_secret)
    jwk = JOSE.JWK.from_oct(@test_secret)

    defaults = %{
      "sub" => "test-user",
      "exp" => DateTime.utc_now() |> DateTime.add(3600) |> DateTime.to_unix(),
      "iat" => DateTime.utc_now() |> DateTime.to_unix()
    }

    merged = Map.merge(defaults, claims)
    {_, token} = JOSE.JWT.sign(jwk, %{"alg" => "HS256"}, merged) |> JOSE.JWS.compact()
    token
  end

  defp auth_conn(method, path, claims) do
    conn(method, path, "")
    |> Plug.Conn.put_req_header("content-type", "application/json")
    |> Plug.Conn.put_req_header("authorization", "Bearer #{sign_jwt(claims)}")
    |> call()
  end

  defp with_auth_enabled(fun) do
    previous = Application.get_env(:api_auth, :auth_disabled)
    Application.delete_env(:api_auth, :auth_disabled)

    on_exit(fn ->
      case previous do
        nil -> Application.delete_env(:api_auth, :auth_disabled)
        value -> Application.put_env(:api_auth, :auth_disabled, value)
      end
    end)

    fun.()
  end

  defp assert_error_body(body, expected_error) do
    assert is_binary(body["message"])
    assert body["error"] == expected_error
    refute body["message"] =~ "%{"
    refute body["message"] =~ "#PID<"
  end

  describe "GET /timer-schedules" do
    test "returns 403 without deploy_bpmn claim" do
      with_auth_enabled(fn ->
        conn = auth_conn(:get, "/timer-schedules", %{})

        assert conn.status == 403
        body = Jason.decode!(conn.resp_body)
        assert_error_body(body, "forbidden")
        assert body["message"] =~ "permissions"
      end)
    end

    test "returns 200 with a data list" do
      with_auth_enabled(fn ->
        conn = auth_conn(:get, "/timer-schedules", %{"deploy_bpmn" => true})

        assert conn.status == 200
        body = Jason.decode!(conn.resp_body)
        assert is_list(body["data"])
      end)
    end
  end

  describe "GET /timer-schedules/:id" do
    test "returns 403 without deploy_bpmn claim" do
      with_auth_enabled(fn ->
        conn = auth_conn(:get, "/timer-schedules/some-schedule-id", %{})

        assert conn.status == 403
        body = Jason.decode!(conn.resp_body)
        assert_error_body(body, "forbidden")
        assert body["message"] =~ "permissions"
      end)
    end

    test "returns 404 for non-existent schedule" do
      with_auth_enabled(fn ->
        schedule_id = "nonexistent-schedule-#{System.unique_integer([:positive])}"

        conn =
          auth_conn(:get, "/timer-schedules/#{schedule_id}", %{"deploy_bpmn" => true})

        assert conn.status == 404
        body = Jason.decode!(conn.resp_body)
        assert_error_body(body, "not_found")
      end)
    end
  end

  describe "PUT /timer-schedules/:id/enable" do
    test "returns 403 without deploy_bpmn claim" do
      with_auth_enabled(fn ->
        conn = auth_conn(:put, "/timer-schedules/some-schedule-id/enable", %{})

        assert conn.status == 403
        body = Jason.decode!(conn.resp_body)
        assert_error_body(body, "forbidden")
        assert body["message"] =~ "permissions"
      end)
    end

    test "returns 404 for non-existent schedule" do
      with_auth_enabled(fn ->
        schedule_id = "nonexistent-schedule-#{System.unique_integer([:positive])}"

        conn =
          auth_conn(:put, "/timer-schedules/#{schedule_id}/enable", %{"deploy_bpmn" => true})

        assert conn.status == 404
        body = Jason.decode!(conn.resp_body)
        assert_error_body(body, "not_found")
      end)
    end
  end

  describe "PUT /timer-schedules/:id/disable" do
    test "returns 403 without deploy_bpmn claim" do
      with_auth_enabled(fn ->
        conn = auth_conn(:put, "/timer-schedules/some-schedule-id/disable", %{})

        assert conn.status == 403
        body = Jason.decode!(conn.resp_body)
        assert_error_body(body, "forbidden")
        assert body["message"] =~ "permissions"
      end)
    end

    test "returns 404 for non-existent schedule" do
      with_auth_enabled(fn ->
        schedule_id = "nonexistent-schedule-#{System.unique_integer([:positive])}"

        conn =
          auth_conn(:put, "/timer-schedules/#{schedule_id}/disable", %{"deploy_bpmn" => true})

        assert conn.status == 404
        body = Jason.decode!(conn.resp_body)
        assert_error_body(body, "not_found")
      end)
    end
  end
end
