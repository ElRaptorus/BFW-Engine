defmodule BfwEngineWeb.Graphql.ModelTypes do
  @moduledoc """
  Absinthe type definitions for the BPMN Model graph (Phase 6.1, WP-2).

  Projects `BfwEngine.BPMN.Model.*` as a first-class, read-only GraphQL
  graph resolved from `BfwEngine.BPMN.ModelCache`. Types here are
  `import_types`-ed into `BfwEngineWeb.Graphql.Schema`, alongside the
  ...
  `BfwEngineWeb.Graphql.ModelResolvers.to_graphql_flow_node/2` — the
  ...
  import BfwEngineWeb.Graphql.ModelSchema.CommonFields
  ...
  @desc "The kind of a BPMN flow node, aligned 1:1 with `BfwEngine.BPMN.Model.FlowNodeData.*`. Event kind (message/timer/error/...) is not folded in here — see the `EventDefinition` union on event-position nodes."
  ...
  @desc "The trigger-specific payload of an event-position flow node (Start/End/IntermediateCatch/IntermediateThrow/Boundary). Exactly one concrete type per `BfwEngine.BPMN.Model.EventDefinition.*` struct."
  ...
  @desc "A polymorphic BPMN flow node. Concrete type is one of the 21 `*Node` types below, aligned 1:1 with `BfwEngine.BPMN.Model.FlowNodeData.*`."
  ...
  @desc "The parsed BPMN process, projected from `BfwEngine.BPMN.Model.Process` via `ModelCache`. Named `ProcessModel` (not `Process`) to avoid colliding with the persistence-layer `Process` GraphQL type."
