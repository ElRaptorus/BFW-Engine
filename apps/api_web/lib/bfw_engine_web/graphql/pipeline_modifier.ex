defmodule BfwEngineWeb.Graphql.PipelineModifier do
  @moduledoc """
  ...
  request so `BFE_GRAPHQL_MAX_COMPLEXITY` takes effect without recompiling.
  ...
  - `BfwEngineWeb.Graphql.Phases.DepthLimit` — rejects queries exceeding
    `BFE_GRAPHQL_MAX_DEPTH` (default 16 levels of field nesting).
  - `BfwEngineWeb.Graphql.Phases.BlockIntrospection` — rejects `__schema`
    and `__type` root fields when `BFE_GRAPHQL_INTROSPECTION_DISABLED=true`.
  """

  alias Absinthe.{Phase, Pipeline}
  alias BfwEngineWeb.Graphql.Phases.{BlockIntrospection, DepthLimit, ErrorLogger}
  ...
  `:api_web, :graphql_max_complexity` (`BFE_GRAPHQL_MAX_COMPLEXITY`).
  """
