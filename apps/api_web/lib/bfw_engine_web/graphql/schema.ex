defmodule BfwEngineWeb.Graphql.Schema do
  @moduledoc """
  Absinthe schema, auto-extended by AshGraphql from every
  Ash resource on the `BfwEngine.Persistence.Api` domain, plus the
  ...
  Safety limits (S-4):
  - Depth limiting via `BfwEngineWeb.Graphql.Phases.DepthLimit` (env:
    `BFE_GRAPHQL_MAX_DEPTH`, default 16, sized for recursive
    `SubProcessNode.flowNodes` — see `docs/architecture/common-pitfalls.md`
    (GraphQL)).
  - Introspection blocking via `BfwEngineWeb.Graphql.Phases.BlockIntrospection`
    (env: `BFE_GRAPHQL_INTROSPECTION_DISABLED`, default false).
  - Complexity limiting is applied at request time by `PipelineModifier`
    (env: `BFE_GRAPHQL_MAX_COMPLEXITY`, default 10000, sized for the Studio
    debugger `dataObjectValues(limit: 500)` snapshot).

  Dataloader is registered as `:model_cache` (see `context/1`), batching
  `BfwEngine.BPMN.ModelCache.fetch/1` per distinct `process_version_id`.
  """

  use Absinthe.Schema

  use AshGraphql, domains: [BfwEngine.Persistence.Api]

  import_types(BfwEngineWeb.Graphql.ModelTypes)
